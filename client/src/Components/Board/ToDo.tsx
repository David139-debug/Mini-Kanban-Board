...
const ToDo = () => {
    ...
    const [taskMap, setTaskMap] = useState<TaskInfo[]>([]);

    useEffect(() => {
        const fetchTasks = async () => {
            let response = await axios.get<TaskInfo[]>("https://jsonplaceholder.typicode.com/todos?_limit=12");
            let data = response.data;
            
            const filtered = data.filter(task => task.id % 3 === 0);
          ...
          <div className="flex flex-col gap-3">
              <div className="flex gap-4 justify-between">
                  <div className="flex gap-2 items-center">
                      <strong className="w-2 h-2 rounded-full bg-[#4F46E5]"></strong>
                      <h1 className="font-bold text-xl" style={{ fontFamily: "Plus Jakarta Sans" }}>To Do <span className="text-[#94A3B8]" style={{ fontFamily: "Plus Jakarta Sans" }}>({taskMap.length})</span></h1>
                  </div>
                  <div className="flex justify-center items-center w-10 h-10 p-4 border border-[#CBD5E1] rounded-full">
                      <button className="cursor-pointer" onClick={() => console.log(taskMap)}>
              ...
              <div className="flex flex-col items-center gap-3">
                  {taskMap.map((task, index) => (
                      <div className="flex flex-col gap-4 p-3 bg-white rounded-3xl border border-[#E2E8F0]" key={index}>
