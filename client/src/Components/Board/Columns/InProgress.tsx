...
const InProgress = ({ progressTasks, setOpenModal }: TodoProps) => {
      ...
      <div className="bg-[#F8FAFC] border border-[#E2E8F0] rounded-4xl p-4">
          <div className="flex flex-col gap-3">
              <div className="flex gap-4 justify-between">
                  ...
                  <div onClick={() => setOpenModal(true)} className="flex justify-center cursor-pointer items-center w-10 h-10 p-4 border border-[#CBD5E1] rounded-full transition duration-100 hover:bg-[#e2ebf3]">
              ...
              <div ref={setNodeRef} className="flex flex-col items-center gap-3">
                  {progressTasks.length === 0 ? (
                      ...
                      progressTasks.map((task) => <DraggableTask key={task.id} task={task} />)
                  )}
              </div>
          </div>
      </div>
