...
const InProgress = ({ progressTasks, setOpenModal, setStatus, dispatch, setEditModal }: TodoProps) => {
          ...
          <div className="flex flex-col gap-3">
              <div className="flex gap-4 justify-between">
                  <div className="flex gap-2 items-center">
                      ...
                      <h1 className="font-bold text-xl">In Progress <span className="text-[#94A3B8]">({progressTasks.length})</span></h1>
                  </div>
                  <div onClick={() => {
                    setOpenModal(true)
                    setStatus("inProgress");
                    }} className="flex justify-center cursor-pointer items-center w-10 h-10 p-4 border border-[#CBD5E1] rounded-full transition duration-100 hover:bg-[#e2ebf3]">
              ...
              <div ref={setNodeRef} className="flex flex-col items-center gap-3">
                  {progressTasks.length === 0 ? (
                      ...
                      progressTasks.map((task) => <DraggableTask setEditModal={setEditModal} dispatch={dispatch} key={task.id} task={task} />)
                  )}
              </div>
          </div>
      </div>
