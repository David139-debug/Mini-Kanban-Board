...
const ToDo = ({ todoTasks, setOpenModal, setStatus, dispatch, setEditModal }: TodoProps) => {
          ...
          <div className="flex flex-col gap-3">
              <div className="flex gap-4 justify-between">
                  <div className="flex gap-2 items-center">
                      ...
                      <h1 className="font-bold text-xl" style={{ fontFamily: "Plus Jakarta Sans" }}>To Do <span className="text-[#94A3B8]" style={{ fontFamily: "Plus Jakarta Sans" }}>({todoTasks.length})</span></h1>
                  </div>
                  <div onClick={() =>{ 
                    setOpenModal(true);
                    setStatus("todo");
                    }} className="flex justify-center items-center w-10 h-10 p-4 border border-[#CBD5E1] cursor-pointer rounded-full transition duration-100 hover:bg-[#e2ebf3]">
              ...
              <div ref={setNodeRef} className="flex flex-col items-center gap-3">
                  {todoTasks.length === 0 ? (
                      ...
                      todoTasks.map((task) => <DraggableTask setEditModal={setEditModal} dispatch={dispatch} key={task.id} task={task} />)
                  )}
              </div>
          </div>
      </div>
