...
const DraggableTask = ({ task, dispatch, setEditModal }: TaskProps) => {
    ...
    const handleDelete = async (id: string) => {
        try {
            await api.delete(`http://localhost:5000/api/deleteTask/${id}`);
    ...
    return (
      <div ref={setNodeRef} style={style} className="flex justify-between flex-row-reverse items-center p-3 bg-white rounded-3xl border border-[#E2E8F0] w-full" key={task.id}>
        <div className='right-3 flex flex-col gap-5 justify-between items-end'>
            ...
            <svg onClick={() => setEditModal({ open: true, id: task.id })} className='cursor-pointer' xmlns="http://www.w3.org/2000/svg" width="25" height="24" viewBox="0 0 576 512">
